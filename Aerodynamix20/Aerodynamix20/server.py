...
def _aerodynamix_browser_html():
    global _AERODYNAMIX_BROWSER_HTML
    if _AERODYNAMIX_BROWSER_HTML is None:
        with open(os.path.join(app.static_folder, 'gust.html'), 'r', encoding='utf-8') as gust_file:
            html = gust_file.read()
        html = html.replace('<title>GUST Browser</title>', '<title>Aerodynamix Browser</title>', 1)
        html = html.replace(
            '<div class="titlebar">',
            """<div class="titlebar">
            <div class="aero-brand" aria-label="Aerodynamix Browser">
                <span class="aero-brand-mark" aria-hidden="true"><i class="fas fa-compass"></i></span>
                <span class="aero-brand-copy">AERODYNAMIX<span class="aero-brand-sub">BROWSER</span></span>
            </div>""",
            1,
        )
        html = re.sub(
            r'<div class="nt-brand" style="position:relative;">.*?</div>\s*<div class="nt-search">',
        ...
        html = html.replace(
            "function poolModeOn() { try { return ls.getItem('gust:wisp:poolmode:v1') !== '0'; } catch { return true; } }",
            """function poolModeOn() {
                try {
                    if (window.AERODYNAMIX_BROWSER &&
                        localStorage.getItem('aerodynamix:wisp:poolmode:v1') !== '1') {
                        return false;
                    }
                    return ls.getItem('gust:wisp:poolmode:v1') !== '0';
                } catch { return false; }
            }""",
            1,
        )
        html = html.replace(
            "Promise.resolve(benchmarkWispPool()).then(rememberFastestRelay, () => {});",
            "if (!window.AERODYNAMIX_BROWSER) Promise.resolve(benchmarkWispPool()).then(rememberFastestRelay, () => {});",
            1,
        )
        bootstrap = """
<script>
(() => {
  window.AERODYNAMIX_BROWSER = true;
  try {
    // The branded edition favors fast first paint. Its pool preference is
    // separate from the original GUST app, so visiting GUST cannot make this
    // version wait for every relay. The settings UI can still opt back in.
    const aeroPoolMode = localStorage.getItem('aerodynamix:wisp:poolmode:v1');
    localStorage.setItem('gust:wisp:poolmode:v1', aeroPoolMode === '1' ? '1' : '0');
    if (!localStorage.getItem('gust:wisp:v1') &&
        !localStorage.getItem('gust:wisp:fastest:v1')) {
      localStorage.setItem('gust:wisp:fastest:v1', 'wss://mages.io/wisp/');
    }
  } catch (_) {}
})();
</script>
<link rel="stylesheet" href="/aerodynamix-browser.css">
"""
        html = html.replace('</head>', bootstrap + '</head>', 1)
        html = html.replace(
            '</body>',
            '<script src="/aerodynamix-browser.js"></script></body>',
            1,
        )
        _AERODYNAMIX_BROWSER_HTML = html
    return _AERODYNAMIX_BROWSER_HTML


@app.route('/aerodynamix-browser/')
@app.route('/aerodynamix-browser.html')
def aerodynamix_browser():
    return Response(_aerodynamix_browser_html(), mimetype='text/html')
