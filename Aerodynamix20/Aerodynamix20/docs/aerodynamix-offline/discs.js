...
(function() {
    ...
    async function tryLaunchGame(gameUrl) {
        const requiresPurchase = gameUrl.includes('game=games/nextbot/');
        if (isPaid()) {
            window.location.href = gameUrl;
            return;
        }
        if (!isFreeTrial() && !requiresPurchase) {
            window.location.href = gameUrl;
            return;
        }
        if (requiresPurchase && !isFreeTrial()) {
            const purchased = await getPurchasedGamesFromServer();
            if (!purchased.some(game => gameIdsMatch(game, 'games/nextbot/'))) {
                window.location.href = '/shop.html?unlock=nextbot';
                return;
            }
        }
