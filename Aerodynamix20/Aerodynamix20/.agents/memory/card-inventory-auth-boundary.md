...
**How to apply:** Preserve the merge during both registration and login, normalize and deduplicate by card ID, and clear the anonymous inventory only after the database write succeeds. Authenticated pack, sell, collection, and trade paths must all use the account inventory.