...
const featured = [
  { file: "run-3.jpg", name: "Run 3", note: "Endless momentum", tag: "Fast start", tint: "#d8f2ff" },
  { file: "drive-mad.jpg", name: "Drive Mad", note: "Unreasonably fun driving", tag: "Popular", tint: "#ffe4d8" },
  { file: "papaspizzeria.png", name: "Papa's Pizzeria", note: "Take the next order", tag: "Time well spent", tint: "#fff0b8" },
  { file: "supersmashflash.jpg", name: "Super Smash Flash", note: "Pick your fighter", tag: "Multiplayer", tint: "#e6ddff" },
];
...
const filters = ["All games", "Quick play", "Classics", "Odd little gems"];

function PlayButton({ label, onClick, quiet = false }: { label: string; onClick: () => void; quiet?: boolean }) {
  return (
    <button
      type="button"
      className={`calm-play-button ${quiet ? "calm-play-button--quiet" : ""}`}
      onClick={onClick}
      aria-label={label}
    >
      <Play size={quiet ? 14 : 16} fill="currentColor" strokeWidth={2.2} />
      {!quiet && <span>Play now</span>}
    </button>
  );
}

export function CalmLibrary() {
  ...
  const shownGames = useMemo(() => {
    ...
    return games.filter(([, name], index) => {
      const matchesSearch = name.toLowerCase().includes(normalized);
      const matchesFilter =
        activeFilter === "All games" ||
        (activeFilter === "Quick play" && index < 12) ||
        (activeFilter === "Classics" && index >= 12 && index < 25) ||
        (activeFilter === "Odd little gems" && index >= 25);
  ...
  const cycleGame = (direction: "previous" | "next") => {
    const currentIndex = games.findIndex(([, name]) => name === playing);
    const nextIndex = currentIndex < 0
      ? 0
      : direction === "next"
        ? (currentIndex + 1) % games.length
        : (currentIndex - 1 + games.length) % games.length;
    startGame(games[nextIndex][1]);
  };

  return (
    <main className="calm-page">
      <style>{`
        .calm-page {
          --calm-ink: #18253a;
          --calm-muted: #718096;
      ...
      <nav className="calm-nav" aria-label="Aerodynamix navigation">
        <div className="calm-shell calm-nav-inner">
          <button className="calm-wordmark" type="button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} aria-label="Return to Aerodynamix home">
            <img src={`${imageRoot}logo.gif`} alt="" />
            <strong><span>A</span>ERODYNAMIX</strong>
          </button>
          <div className={`calm-nav-links ${menuOpen ? "is-open" : ""}`}>
            {["Games", "Apps", "Media Player", "Connect"].map((item, index) => (
              <button type="button" className={index === 0 ? "is-active" : ""} key={item} onClick={() => setMenuOpen(false)}>{item}</button>
            ))}
            <button type="button" onClick={() => { setSettingsOpen((open) => !open); setMenuOpen(false); }}>
              <Settings2 size={13} /> Settings
            </button>
          </div>
          <button className="calm-menu-button" type="button" onClick={() => setMenuOpen((open) => !open)} aria-expanded={menuOpen} aria-label="Toggle navigation menu">
            {menuOpen ? <X size={15} /> : <Menu size={15} />} {menuOpen ? "Close" : "Menu"}
          </button>
        </div>
      </nav>

      {settingsOpen && (
        <section className="calm-settings-panel" aria-label="Settings panel">
          <h3>Small preferences</h3>
          <div className="calm-setting-line"><span>Reduce motion</span><button className={`calm-toggle ${reduceMotion ? "is-on" : ""}`} type="button" aria-label="Toggle reduce motion" aria-pressed={reduceMotion} onClick={() => setReduceMotion((enabled) => !enabled)} /></div>
          <div className="calm-setting-line"><span>Show favorites first</span><button className={`calm-toggle ${favoritesFirst ? "is-on" : ""}`} type="button" aria-label="Toggle favorites first" aria-pressed={favoritesFirst} onClick={() => setFavoritesFirst((enabled) => !enabled)}>{favoritesFirst && <Check size={11} color="white" />}</button></div>
          <div className="calm-setting-line"><span>Library layout</span><strong>Grid</strong></div>
        </section>
      )}

      <div className="calm-shell calm-main">
        <div className="calm-kicker"><Sparkles size={13} /> A quieter way to play</div>
        <header className="calm-hero-head">
          <h1>Good games,<br /><em>no searching required.</em></h1>
          <p className="calm-hero-copy">A private-feeling corner of the internet for the games you already love — and the ones you are about to.</p>
        </header>

        <section aria-labelledby="spotlight-title">
          <div className="calm-featured">
            <article className="calm-featured-main">
              <img src={`${imageRoot}${featured[0].file}`} alt={featured[0].name} />
              <div className="calm-featured-content">
                <div>
                  <span className="calm-featured-pill">Featured · {featured[0].tag}</span>
                  <h2 id="spotlight-title">{featured[0].name}</h2>
                  <p>{featured[0].note}</p>
                </div>
                <button className="calm-featured-play" type="button" onClick={() => startGame(featured[0].name)}>
                  <Play size={15} fill="currentColor" /> Play
                </button>
              </div>
            </article>
            <div className="calm-featured-side" aria-label="More featured games">
              {featured.slice(1).map((game) => (
                <article className="calm-mini-card" key={game.name}>
                  <img src={`${imageRoot}${game.file}`} alt={game.name} />
                  <button type="button" className="calm-play-button calm-play-button--quiet" onClick={() => startGame(game.name)} aria-label={`Play ${game.name}`}>
        ...
        <section className="calm-library" aria-labelledby="library-title">
          <div className="calm-library-top">
            <div>
              <p className="calm-section-label">The full collection</p>
              <h2 id="library-title">Find your next game</h2>
              <div className="calm-library-meta"><Library size={13} /> {shownGames.length} titles ready to play</div>
            </div>
            <label className="calm-search">
              <Search size={16} />
              <input value={query} onChange={(event) => setQuery(event.target.value)} type="search" placeholder="Search the library" aria-label="Search games" />
              {query && <button type="button" onClick={() => setQuery("")} aria-label="Clear search"><X size={14} /></button>}
            </label>
          </div>

          <div className="calm-filter-row" aria-label="Filter games">
            <SlidersHorizontal size={14} color="#9aaabd" aria-hidden="true" />
            {filters.map((filter) => (
              <button type="button" className={`calm-filter ${activeFilter === filter ? "is-active" : ""}`} key={filter} onClick={() => setActiveFilter(filter)}>
                {filter}
                {filter === "All games" && <span className="calm-filter-count">{games.length}</span>}
              </button>
            ))}
            <span style={{ marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 6, color: "#9aaabd", fontSize: 10, whiteSpace: "nowrap" }}>
              <Grid2X2 size={13} /> Curated grid
            </span>
          </div>

          <div className="calm-game-grid">
            {shownGames.map(([file, name], index) => {
              const loved = favorites.includes(name);
              return (
                <article className="calm-game-card" key={file}>
                  <div className="calm-game-art">
                    <img src={`${imageRoot}${file}`} alt={name} loading={index < 12 ? "eager" : "lazy"} />
                    <button className={`calm-favorite ${loved ? "is-loved" : ""}`} type="button" onClick={() => toggleFavorite(name)} aria-label={`${loved ? "Remove" : "Add"} ${name} ${loved ? "from" : "to"} favorites`} aria-pressed={loved}>
                      <Heart size={13} fill={loved ? "currentColor" : "none"} />
                    </button>
                    <div className="calm-game-hover">
                      <PlayButton label={`Play ${name}`} onClick={() => startGame(name)} quiet />
                    </div>
                  </div>
                  <span className="calm-game-name" title={name}>{name}</span>
                  <span className="calm-game-type">{index % 3 === 0 ? "Arcade" : index % 3 === 1 ? "Classic" : "Browser game"}</span>
                </article>
              );
            })}
            {shownGames.length === 0 && (
              <div className="calm-empty">
                <div>
                  <Search size={20} color="#9aaabd" />
                  <strong>No games in that corner.</strong>
      ...
      <aside className="calm-player" aria-label="Persistent media player">
        <div className="calm-shell calm-player-inner">
          <div className="calm-track">
            <img className="calm-track-art" src={`${imageRoot}${playing ? (games.find(([, name]) => name === playing)?.[0] ?? "logo.gif") : "logo.gif"}`} alt="" />
            <div className="calm-track-meta">
              <strong>{playing ?? "Nothing queued"}</strong>
              <span>{playing ? "Game session" : "Media Player · ready when you are"}</span>
            </div>
          </div>
          <div className="calm-player-controls">
            <button type="button" aria-label="Previous track" onClick={() => cycleGame("previous")}><ChevronLeft size={16} /></button>
            <button className="calm-player-main" type="button" aria-label={isPaused ? "Resume media" : "Pause media"} onClick={() => setIsPaused((paused) => !paused)}>
              {isPaused ? <Play size={15} fill="currentColor" /> : <Pause size={15} fill="currentColor" />}
            </button>
            <button type="button" aria-label="Next track" onClick={() => cycleGame("next")}><ChevronRight size={16} /></button>
          </div>
          <div className="calm-player-progress">
            <span>{playing ? "0:00" : "0:00"} / {playing ? "—" : "0:00"}</span>
            <input className="calm-range" type="range" min="0" max="100" defaultValue="14" aria-label="Track progress" />
            <div className="calm-volume">
              <Volume2 size={14} />
              <input className="calm-range" type="range" min="0" max="100" value={volume} onChange={(event) => setVolume(Number(event.target.value))} aria-label="Volume" />
            </div>
            <button type="button" aria-label="Close media player" onClick={() => setPlaying(null)} style={{ display: "grid", placeItems: "center", border: 0, color: "#8493a5", background: "transparent", cursor: "pointer" }}><X size={14} /></button>
          </div>
        </div>
      </aside>
    </main>
