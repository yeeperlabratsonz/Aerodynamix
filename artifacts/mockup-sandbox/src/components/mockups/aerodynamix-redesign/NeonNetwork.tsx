...
const featured = [
  { file: "run-3.jpg", name: "Run 3", category: "Arcade", signal: "98.4" },
  { file: "drive-mad.jpg", name: "Drive Mad", category: "Racing", signal: "91.7" },
  { file: "papaspizzeria.png", name: "Papa's Pizzeria", category: "Casual", signal: "86.3" },
  { file: "supersmashflash.jpg", name: "Super Smash Flash", category: "Action", signal: "83.9" },
];

const games = [
  ["run-3.jpg", "Run 3", "Arcade"],
  ["drive-mad.jpg", "Drive Mad", "Racing"],
  ["retro-bowl.jpg", "Retro Bowl", "Sports"],
  ["slope.jpg", "Slope", "Arcade"],
  ["mc.png", "Minecraft", "Sandbox"],
  ["supersmashflash.jpg", "Super Smash Flash", "Action"],
  ["papaspizzeria.png", "Papa's Pizzeria", "Casual"],
  ["papasfreezeria.png", "Papa's Freezeria", "Casual"],
  ["papas-pancakeria.png", "Papa's Pancakeria", "Casual"],
  ["papas-bakeria.png", "Papa's Bakeria", "Casual"],
  ["meat-boy.png", "Meat Boy", "Platformer"],
  ["newgrounds-rumble.png", "Newgrounds Rumble", "Action"],
  ["we-become-what-we-behold.png", "We Become What We Behold", "Puzzle"],
  ["bad-time-simulator.png", "Bad Time Simulator", "Action"],
  ["deltarune.png", "Deltarune", "RPG"],
  ["adventure-capitalist.png", "Adventure Capitalist", "Strategy"],
  ["fridaynightfunkin.png", "Friday Night Funkin'", "Rhythm"],
  ["run-2.png", "Run 2", "Arcade"],
  ["picoschool.png", "Pico's School", "Adventure"],
  ["worldshardestgame.png", "World's Hardest Game", "Challenge"],
  ["sandboxels.png", "Sandboxels", "Sandbox"],
  ["alien-hominid.png", "Alien Hominid", "Action"],
  ["subway-surfers-sf.jpg", "Subway Surfers San Francisco", "Arcade"],
  ["hobo-1.png", "Hobo 1", "Action"],
  ["hobo-2.png", "Hobo 2", "Action"],
  ["hobo-3.png", "Hobo 3", "Action"],
  ["hobo-4.png", "Hobo 4", "Action"],
  ["hobo-5.png", "Hobo 5", "Action"],
  ["hobo-6.png", "Hobo 6", "Action"],
  ["hobo-7.png", "Hobo 7", "Action"],
  ["gladihoppers.jpg", "Gladihoppers", "Action"],
  ["fruit-ninja.png", "Fruit Ninja", "Arcade"],
  ["binding-of-isaac.png", "Binding of Isaac", "RPG"],
  ["crossy-road.png", "Crossy Road", "Arcade"],
  ["cookie-clicker.png", "Cookie Clicker", "Idle"],
  ["duck-life.png", "Duck Life", "Casual"],
  ["geometry-dash-lite.jpg", "Geometry Dash Lite", "Rhythm"],
  ["doom.png", "Doom", "Action"],
  ["doki-doki-literature-club.jpg", "Doki Doki Literature Club", "RPG"],
  ["baldis-basics-classic-remastered.png", "Baldi's Basics Classic Remastered", "Challenge"],
  ["stickmin-breaking-bank.jpg", "Breaking the Bank", "Adventure"],
  ["stickmin-escaping-prison.avif", "Escaping the Prison", "Adventure"],
  ["stickmin-stealing-diamond.avif", "Stealing the Diamond", "Adventure"],
  ["stickmin-infiltrating-airship.avif", "Infiltrating the Airship", "Adventure"],
  ["stickmin-fleeing-complex.avif", "Fleeing the Complex", "Adventure"],
  ["greatest-game-square.svg", "The Greatest Game of All Time", "Special"],
  ["nubbys-number-factory.jpg", "Nubby's Number Factory", "Strategy"],
] as const;

const categories = ["All", "Arcade", "Action", "Casual", "Adventure", "Challenge"];
...
export function NeonNetwork() {
  ...
  const [selectedGame, setSelectedGame] = useState("Run 3");
  ...
  const shownGames = useMemo(
    () =>
      games.filter(([, name, gameCategory]) => {
        const matchesQuery = name.toLowerCase().includes(query.toLowerCase());
        const matchesCategory = category === "All" || gameCategory === category;
    ...
    <main className="neon-network">
      <style>{`
        .neon-network {
          ...
          padding-bottom: ${playerOpen ? "92px" : "24px"};
        }
        ...
        @media (max-width: 760px) {
          ...
          .nn-nav { position: absolute; top: 58px; right: 15px; left: 15px; display: ${menuOpen ? "flex" : "none"}; flex-direction: column; align-items: stretch; padding: 7px; border: 1px solid var(--nn-line); border-radius: 10px; background: #0b1729; box-shadow: 0 18px 35px rgba(0,0,0,.4); }
      ...
      <div className="nn-noise" />
      <header className="nn-topbar">
        <div className="nn-shell nn-topbar-inner">
          <button className="nn-brand" type="button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>
            <img className="nn-logo" src={`${imageRoot}logo.gif`} alt="Aerodynamix" />
            <span className="nn-brand-word"><b>A</b>ERODYNAMIX</span>
          </button>
          <nav className="nn-nav" aria-label="Aerodynamix navigation">
            {["Games", "Apps", "Media Player", "Connect"].map((item, index) => (
              <button key={item} className={index === 0 ? "active" : ""} type="button">
                {item}
              </button>
            ))}
          </nav>
          <button className="nn-settings" type="button"><Settings size={14} /> Settings</button>
          <div className="nn-menu">
            <button type="button" onClick={() => setMenuOpen((open) => !open)} aria-expanded={menuOpen}>
              {menuOpen ? <X size={14} /> : <Menu size={14} />} {menuOpen ? "Close" : "Menu"}
            </button>
          </div>
        </div>
      </header>
      ...
      <div className="nn-shell">
        <section className="nn-intro" aria-labelledby="network-title">
          <div>
            <div className="nn-kicker"><i /> Network online / 04:12:08 <Check size={12} /></div>
            <h1 className="nn-title" id="network-title">Find your next<br /><em>instant launch.</em></h1>
            <p className="nn-subtitle">A private arcade of sharp little worlds. Scan the network, save what catches, and launch without the detour.</p>
            <button className={`nn-scan ${scanned ? "done" : ""}`} type="button" onClick={() => setScanned(true)}>
              {scanned ? <Check size={15} /> : <Zap size={15} />} {scanned ? "NETWORK SCANNED" : "SCAN FOR SIGNALS"}
            </button>
          </div>
          <aside className="nn-status-panel" aria-label="Network status">
            <div className="nn-status-head"><span>Collection status</span><span>Live</span></div>
            <div className="nn-readout"><strong>47</strong><span>games<br />indexed</span></div>
            <div className="nn-meter"><i /></div>
            <div className="nn-status-foot"><span>Availability</span><b>100% local</b></div>
          </aside>
        </section>

        <section className="nn-network-map" aria-labelledby="spotlight-heading">
          <div className="nn-map-lines" />
          <div className="nn-map-label"><b>SPOTLIGHT /</b> HIGH SIGNAL ROUTES</div>
          <article className="nn-featured-node" onClick={() => launchGame(featured[0].name)} tabIndex={0} role="button" onKeyDown={(event) => event.key === "Enter" && launchGame(featured[0].name)}>
            <img src={`${imageRoot}${featured[0].file}`} alt={featured[0].name} />
            <div className="nn-node-copy">
              <div><h2 id="spotlight-heading">{featured[0].name}</h2><small>{featured[0].category} / SIGNAL {featured[0].signal}</small></div>
              <button className="nn-launch" type="button" aria-label={`Launch ${featured[0].name}`} onClick={(event) => { event.stopPropagation(); launchGame(featured[0].name); }}><Play size={15} fill="currentColor" /></button>
            </div>
          </article>
          <div className="nn-mini-nodes">
            {featured.slice(1, 3).map((game) => (
              <article className="nn-mini-card" key={game.name} onClick={() => launchGame(game.name)} tabIndex={0} role="button" onKeyDown={(event) => event.key === "Enter" && launchGame(game.name)}>
                <img src={`${imageRoot}${game.file}`} alt={game.name} />
                <div className="nn-mini-copy"><h3>{game.name}</h3><span>{game.category} / {game.signal}</span></div>
              </article>
            ))}
          </div>
          <div className="nn-map-aside">
            <div><div className="nn-kicker"><Sparkles size={12} /> New route</div><h3>One clean path to play.</h3><p>Every node is ready when you are. No accounts, no waiting room.</p></div>
            <div className="nn-route">ROUTE READY</div>
          </div>
        </section>

        <section aria-labelledby="library-heading">
          <div className="nn-section-bar">
            <div><h2 id="library-heading">Game library</h2><p>Browse the full collection by signal type.</p></div>
            <span className="nn-library-meta">{shownGames.length.toString().padStart(2, "0")} / {games.length} NODES</span>
          </div>
          <div className="nn-toolbar">
            <label className="nn-search"><Search size={16} /><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search the network" aria-label="Search games" /></label>
            <div className="nn-filter" aria-label="Filter games">
              {categories.map((item) => <button key={item} type="button" className={category === item ? "active" : ""} onClick={() => setCategory(item)}>{item}</button>)}
            </div>
            <button type="button" aria-label="Library view options" className="nn-settings"><SlidersHorizontal size={15} /></button>
          </div>
          <div className="nn-grid">
            {shownGames.length ? shownGames.map(([file, name, gameCategory]: Game, index) => (
              <article className="nn-game" key={file} onClick={() => launchGame(name)} tabIndex={0} role="button" onKeyDown={(event) => event.key === "Enter" && launchGame(name)}>
                <img src={`${imageRoot}${file}`} alt={name} loading={index > 5 ? "lazy" : "eager"} />
                <div className="nn-game-info">
                  <div><h3>{name}</h3><span>{gameCategory}</span></div>
                  <button type="button" className={`nn-fav ${favorites.includes(name) ? "saved" : ""}`} aria-label={`${favorites.includes(name) ? "Remove" : "Save"} ${name}`} onClick={(event) => { event.stopPropagation(); toggleFavorite(name); }}>
                    <Heart size={13} fill={favorites.includes(name) ? "currentColor" : "none"} />
                  </button>
                </div>
              </article>
            )) : <div className="nn-empty">No nodes match that scan. Try another title or signal type.</div>}
          </div>
        </section>
      </div>

      {playerOpen && (
        <aside className="nn-player" aria-label="Persistent media player">
          <div className="nn-shell nn-player-inner">
            <div className="nn-track">
              <img className="nn-track-art" src={`${imageRoot}${(games.find(([, name]) => name === selectedGame) ?? games[0])[0]}`} alt="" />
              <div className="nn-track-meta"><strong>{selectedGame}</strong><span>{isPlaying ? "Launch signal active" : "Ready to launch"}</span></div>
              <span className="nn-live">{isPlaying ? "LIVE" : "READY"}</span>
            </div>
            <div className="nn-controls">
              <button type="button" aria-label="Previous game" onClick={() => setSelectedGame(featured[0].name)}><ChevronLeft size={17} /></button>
              <button className="nn-play" type="button" aria-label={isPlaying ? "Pause" : "Play"} onClick={() => setIsPlaying((playing) => !playing)}>{isPlaying ? <Pause size={16} fill="currentColor" /> : <Play size={16} fill="currentColor" />}</button>
              <button type="button" aria-label="Next game" onClick={() => setSelectedGame(featured[1].name)}><ChevronRight size={17} /></button>
            </div>
            <div className="nn-progress"><span>{isPlaying ? "0:12" : "0:00"} / 0:45</span><input type="range" min="0" max="100" defaultValue="24" aria-label="Track progress" /><Volume2 size={14} /><input type="range" min="0" max="100" value={volume} onChange={(event) => setVolume(Number(event.target.value))} aria-label="Volume" /><button className="nn-close" type="button" aria-label="Close player" onClick={() => setPlayerOpen(false)}><X size={15} /></button></div>
          </div>
        </aside>
      )}
      {!playerOpen && <button type="button" onClick={() => setPlayerOpen(true)} style={{ position: "fixed", right: 16, bottom: 16, zIndex: 30, display: "grid", placeItems: "center", width: 42, height: 42, border: "1px solid rgba(104,230,223,.45)", borderRadius: "50%", color: "#071019", background: "#c7ee65", cursor: "pointer" }} aria-label="Open media player"><LayoutGrid size={17} /></button>}
    </main>
  );
}